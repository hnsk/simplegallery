...
7. **Frontend (CSS/JS)** — done. JS: `Lightbox._build()` injects `<a class="lightbox-download" download hidden>↓</a>` next to close. New `_setDownload(item)` populates `href` from `item.original` (read via `figure.dataset.original` in `figureToItem`) and sets the `download` attr to the basename derived from the URL (`filenameFromPath`); hidden when no original. CSS: dropped dead `.index-grid` rules; added `.breadcrumbs` (flex `<ol>` with `/` separator + bold `aria-current` page), `.subgallery-grid` (auto-fill 220px), `.subgallery-card` (hover lift), `.subgallery-card--text` (text-only variant, no thumb), `.lightbox-download` (positioned `right: 4rem; top: 1rem`, font-size 1.5rem). `tests/test_frontend_assets.py` pins all new CSS/JS selectors. Suite: 113 pass, 1 skip, 2 pre-existing sample-data flakes (`129679.jpg`, `shelf-christmas-decoration.heic`) — no new regressions.
...
Next substep: **Watcher (10.8)** — port `WatcherService` + `GalleryEventHandler` to the recursive layout. Dirty unit becomes the source-dir POSIX rel path (relative to `config.source`) of the changed file. Dir create/delete/move marks both endpoints + their parent. Re-introduce per-dir dirty propagation in `GalleryBuilder.build_galleries(...)` so the partial rebuild path doesn't fall back to a full `build_tree()` on every event. Verify the existing `FileOpenedEvent` / `FileClosedNoWriteEvent` filter still prevents the rebuild loop with output writes confined to `web/<rel>/{thumbs,full,video}/`. Update `tests/test_watcher.py` for nested events.
...
- Tests: `docker compose run --rm test` (113 pass, 1 skip, 2 pre-existing sample-data flakes)
